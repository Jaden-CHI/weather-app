...
class AppScheduleService {
  ...
  Future<void> addFishingSchedule({
    required String title,
  ...
  Future<List<FishingEvent>> getUpcomingFishingSchedules() async {
    try {
      debugPrint('📍 Getting fishing schedules for UID: $_uid');
      final snapshot = await _database
          .child(_schedulesPath)
          .child(_uid)
          .get()
      ...
      final events = <FishingEvent>[];
      final data = snapshot.value as Map<dynamic, dynamic>;
      debugPrint('📊 Found ${data.length} schedules');

      data.forEach((scheduleId, scheduleData) {
        final schedule = Map<String, dynamic>.from(scheduleData);
        if (schedule['type'] == 'fishing') {
          events.add(FishingEvent(
            id: scheduleId,
            title: schedule['title'],
            startDate: DateTime.fromMillisecondsSinceEpoch(schedule['startAt']),
      ...
      events.sort((a, b) => a.startDate.compareTo(b.startDate));
      debugPrint('✅ Got ${events.length} fishing events');
      return events;
    } catch (e) {
      debugPrint('❌ Fishing schedules fetch error: $e');
      return [];
    }
  }
